...
const subModuleDeskripsiPretest = (subModule: string) => {
  if (subModule === "TWK") {
      ...
      <section className="relative w-[100%]  p-4 md:p-5">
        <div className="mb-3">
          <h1 className="text-md font-bold">
            Pretest {dataAllPretest?.module_name}
          </h1>
          <h2 className="text-sm">
            {dataAllPretest?.module_name === "SKD"
              ? "Berikut adalah pretest modul SKD(Seleksi Kompentensi Dasar) yang di pisah per-submodul"
          ...
          <div className="w-[70%]">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              {dataAllPretest?.quizzes_grouped_by_sub_module.map(
                (dataPretest, index) => (
                  <Link
                    href={`${params.module_id}/daftar-pretest/${dataPretest.sub_module_uuid}`}
                    key={index}
                    style={{
                      backgroundColor:
                        srcItemCardsMap[dataPretest.sub_module_name].color,
                    }}
                    className={`rounded-md py-6 `}
                  >
                    <div className="mb-3 flex justify-between px-6 text-sm">
                      <h3>Pretest {dataPretest.sub_module_name}</h3>
                      <ArrowCircleRightIcon sx={{ color: "white" }} />
                    </div>

                    <h1 className="text-md px-6 font-bold">
                      {subModuleDeskripsiPretest(dataPretest.sub_module_name)}
                    </h1>
                    <h3 className="px-6 text-xs">
                      {dataPretest.quiz_count} Pertemuan |{" "}
                      {dataPretest.quiz_count} Pretest
                    </h3>
                    <div className="mt-3 flex justify-between">
                      <div className="flex w-[50%] flex-col items-center justify-center">
                        <div className="h-16 w-16">
                          <CircularProgressbar
                            value={percentage(
                              dataPretest.quiz_count,
                              dataPretest.submission_count,
                            )}
                            text={`${percentage(
                              dataPretest.quiz_count,
                              dataPretest.submission_count,
                            ...
                            styles={buildStyles({
                              ...
                              // Colors
                              pathColor:
                                srcItemCardsMap[dataPretest.sub_module_name]
                                  .iconColor,
                              textColor: "#000000",
                              // trailColor: "#d6d6d6",
                        ...
                        <div className="ml-2 mt-3 min-h-5 w-fit rounded-md bg-white p-1 pr-2 shadow-md">
                          <h3 className="text-xs">
                            ...
                            />
                            {dataPretest.submission_count} of{" "}
                            {dataPretest.quiz_count}
                          </h3>
                        </div>
                      </div>
